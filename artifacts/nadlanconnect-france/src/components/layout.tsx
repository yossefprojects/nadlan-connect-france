        ...
        >
          <div className="container flex h-16 items-center justify-between gap-4">
              ...
              <nav className="hidden lg:flex gap-5" aria-label={t.navAria}>
                {navConfig.filter((item) => item.key !== "simu").map((item) => {
                  ...
                  return (
                    <Link
                      key={item.href}
                      href={item.href}
                      className={`relative text-[13px] font-medium whitespace-nowrap transition-colors hover:text-foreground px-1 py-2 ${
                        isActive ? "text-foreground" : "text-foreground/65"
                      }`}
                    >
                      {t.nav[item.key]}
                      {isActive && (
                        <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-sea" />
                      )}
                    </Link>
                  );
                })}
                ...
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className={`relative flex items-center gap-1 text-[13px] font-medium whitespace-nowrap transition-colors hover:text-foreground px-1 py-2 ${
                      isProActive ? "text-foreground" : "text-foreground/65"
                    }`}
                  >
                    {t.pros}
                    <ChevronDown className={`w-3 h-3 opacity-50 transition-transform duration-200 ${proDropdown ? "rotate-180" : ""}`} />
                    {isProActive && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-sea" />}
                  </button>
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              {/* Language toggle */}
              <div className="flex items-center rounded-full border border-border overflow-hidden text-[11px] font-bold">
                {(["fr", "en"] as const).map((l) => (
                  <button
                    ...
                    className={`px-2.5 py-[5px] transition-colors duration-150 ${
                      lang === l ? "bg-primary text-primary-foreground" : "text-foreground/55 hover:text-foreground"
                    }`}
                  >
                    {l.toUpperCase()}
                  </button>
              ...
              <button onClick={() => setModalOpen(true)}
                ...
                className="hidden xl:flex items-center gap-1.5 px-3.5 py-1.5 rounded-full border border-border text-foreground/70 text-[13px] font-medium hover:border-sea hover:text-foreground transition-all duration-150 whitespace-nowrap">
              ...
              <Link href="/simulateur"
                className="hidden sm:flex items-center gap-1.5 h-9 px-5 rounded-full bg-primary hover:bg-ink-2 text-primary-foreground font-bold text-[13px] whitespace-nowrap shrink-0 shadow-sm transition-all hover:-translate-y-0.5 border-0">
                {t.simuCta}
              </Link>

              {/* Mobile menu toggle */}
              <button className="lg:hidden text-foreground/80 hover:text-foreground hover:bg-foreground/5 rounded-full h-9 w-9 flex items-center justify-center shrink-0"
