      ...
      <section className="relative h-[36vh] min-h-[240px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 pb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/15 bg-white/5 text-white/70 text-xs font-medium mb-3">
            <span className="w-1.5 h-1.5 rounded-full bg-[#C9A84C]" /> {t.hero.badge}
          </div>
            ...
            <div className="flex overflow-x-auto">
              {t.tabs.map((tabItem, i) => (
                <button key={tabItem.id} onClick={() => setTab(tabItem.id)}
                  className={`flex items-center gap-1.5 px-3 sm:px-5 py-3.5 sm:py-4 text-xs sm:text-sm font-semibold whitespace-nowrap border-b-2 transition-colors flex-1 sm:flex-none justify-center sm:justify-start ${tab === tabItem.id ? "border-[#C9A84C] text-[#1E3A5F]" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    {Number(prix) > 0 && Number(surface) > 0 && (
                      <div className="mt-4 grid grid-cols-3 gap-3">
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.bien.prixM2}</p><p className="font-black text-[#1E3A5F] text-sm">{fmtNum(Math.round(Number(prix)/Number(surface)))}€</p></div>
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.bien.fraisNotaire}</p><p className="font-black text-[#1E3A5F] text-sm">{fmtNum(fraisNotaire)}€</p></div>
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.bien.coutTotal}</p><p className="font-black text-[#C9A84C] text-sm">{fmtNum(prixTotal)}€</p></div>
                      </div>
                    )}
                  </div>
              ...
              {tab === "revenus" && (
                <div className="space-y-5">
                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    <h2 className="text-sm font-bold text-[#1E3A5F] uppercase tracking-widest mb-5">{t.revenus.heading}</h2>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="col-span-2">
                    ...
                    {Number(loyer) > 0 && (
                      <div className="mt-4 grid grid-cols-3 gap-3">
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.revenus.loyerAnnuel}</p><p className="font-black text-[#1E3A5F] text-sm">{fmtNum(loyerAnnuel)}€</p></div>
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.revenus.apresVacance(tauxVacance)}</p><p className="font-black text-[#1E3A5F] text-sm">{fmtNum(Math.round(loyerAnnuel * (1 - Number(tauxVacance)/100)))}€</p></div>
                        <div className="bg-[#f8f9fb] rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 mb-0.5">{t.revenus.rendementBrut}</p><p className="font-black text-[#C9A84C] text-sm">{rendBrut}%</p></div>
                      </div>
                    )}
                  </div>

                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    <h2 className="text-sm font-bold text-[#1E3A5F] uppercase tracking-widest mb-5">{t.revenus.chargesHeading}</h2>
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="mt-4 p-4 bg-[#f8f9fb] rounded-xl border border-gray-100">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-black text-[#1E3A5F]">{fmtNum(chargesTotal)}{t.units.euroYear}</span>
                      </div>
              ...
              {tab === "fiscalite" && (
                <div className="space-y-5">
                  <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    <h2 className="text-sm font-bold text-[#1E3A5F] uppercase tracking-widest mb-5">{t.fiscalite.heading}</h2>
                        ...
                        <div className="grid grid-cols-5 gap-1.5">
                          {TMI_OPTIONS.map((tmiOpt) => (
                            <button key={tmiOpt} onClick={() => setTmi(String(tmiOpt))}
                              className={`py-2 rounded-xl text-xs font-bold border transition-all ${tmi === String(tmiOpt) ? "bg-[#1E3A5F] text-white border-[#1E3A5F]" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
                              {tmiOpt}%
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="bg-[#1E3A5F]/5 border border-[#1E3A5F]/15 rounded-2xl p-5">
                    <h3 className="text-xs font-bold text-[#1E3A5F] uppercase tracking-widest mb-3 flex items-center gap-2">
                      <Info className="w-3.5 h-3.5 text-[#C9A84C]" /> {t.fiscalite.pressionHeading}
                    </h3>
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div><p className="text-[10px] text-gray-400 mb-1">{t.fiscalite.tmi}</p><p className="font-black text-[#1E3A5F]">{tmi}%</p></div>
                      <div><p className="text-[10px] text-gray-400 mb-1">{t.fiscalite.prelevements}</p><p className="font-black text-[#1E3A5F]">17,2%</p></div>
                    ...
                    {dispositif === "jeanbrun" && (
                      <p className="text-xs text-gray-500 mt-3 leading-relaxed">
                        <strong className="text-[#1E3A5F]">{t.fiscalite.jeanbrunLabel}</strong> {t.fiscalite.jeanbrunText1}<strong>{t.fiscalite.jeanbrunBold}</strong>{t.fiscalite.jeanbrunText2}
                      </p>
                    )}
                    {dispositif === "lmnp_reel" && (
                      <p className="text-xs text-gray-500 mt-3 leading-relaxed">
                        <strong className="text-[#1E3A5F]">{t.fiscalite.lmnpLabel}</strong> {t.fiscalite.lmnpText1}<strong>{t.fiscalite.lmnpBold}</strong>{t.fiscalite.lmnpText2}
                      </p>
                    )}
                  </div>
                ...
                <div>
                  {loading && (
                    <div className="bg-white rounded-2xl border border-gray-100 p-12 flex flex-col items-center text-center">
                      ...
                      <p className="font-bold text-[#1E3A5F] text-lg mb-1">{t.resultats.loadingTitle}</p>
                        ...
                        {results.synthese_fiscale && (
                          <div className="bg-[#1E3A5F]/5 border border-[#1E3A5F]/10 rounded-xl p-4">
                            <p className="text-xs font-semibold text-[#1E3A5F] uppercase tracking-widest mb-2 flex items-center gap-1.5">
                              <BarChart3 className="w-3.5 h-3.5 text-[#C9A84C]" /> {t.resultats.syntheseFiscale}
                            </p>
